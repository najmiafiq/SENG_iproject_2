...
class Sprite{
    constructor({ position, imageSrc, scale = 1,
        maxFrames = 1, holdFrames = 30, offsetFrame = { x: 0, y: 0 }
    }) {
    ...
    draw() {
        //Draw image in the canvas with own crop properties
    ...
    animateFrames() {
        this.elapsedFrames++;
        //Update the current frame only if we wait holdFrames
        if (this.elapsedFrames % this.holdFrames === 0) {
            if (this.currentFrame < this.maxFrames - 1) {
                ...
                if (this instanceof Fighter) {
                    if (this.health > 0)
                        this.currentFrame = 0; //Reset chara frame to beginning
                } else
                    this.currentFrame - 0; //Reset frame to beginning
            }
        }
    }
