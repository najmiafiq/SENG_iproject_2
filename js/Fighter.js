...
class Fighter extends Sprite{
    constructor({ name, position, offset, imageSrc, scale, maxFrames, holdFrames, offsetFrame = { x: 0, y: 0 }, sprites, keys, attackTime }) {
        ...
        this.moveFactor = 5;    //Determinates how fast this sprite can move due to user input.
    ...
    attack(enemyFighter) {
        if (this.isAttacking && this.health > 0 && this.attackCooldown) {
            ...
            setTimeout(() => { this.attackCooldown = true }, 500);
    ...
    movement() {
        //Determine if character is moving to the left or right
        let running = false;
        if (this.health > 0) {
    ...
    update() {
        ...
        if (this.position.y + this.height + this.velocity.y >= canvas.height - 95) { // Sprite reach the bottom of the canvas.
            ...
            if (this.velocity.y > 0) {  //Sprite's y velocity is positive, it's falling.
                ...
                this.inTheAir = true;   //Sprite is in the air and can't jump again.
...
export const player1 = new Fighter({
    name: "P1",
    position: {
        x: 0,
        y: 0
...
// Create enemy sprite.
export const player2 = new Fighter({
    name: "P2",
    position: {
        x: 950,
        y: 0
